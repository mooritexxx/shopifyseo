...
def _catalog_image_row(
    *,
    ...
    # Status checkmark uses heuristic only (original behavior). SEO template mismatch is separate so
    # a successful rename+sync does not stay "red" when the CDN name differs from the template slug.
...
def _passes_filters(
    ...
    *,
    ...
    # status filter must match summary counts + UI status column (heuristic weak_filename only).
    # seo_filename_mismatch is tracked separately so CDN/template drift does not flip the checkmark.

    not_seo_optimized = miss_alt or weak_fn or not_webp
...
def _list_catalog_image_seo_rows_impl(
    ...
    *,
    ...
    def sort_key(x: dict[str, Any]) -> tuple:
        ...
        seo_ok = not miss and not weak and not not_webp
    ...
    for it in items:
        f = it.get("flags") or {}
        if not f.get("missing_or_weak_alt") and not f.get("weak_filename") and not f.get("not_webp"):
            summary["optimized"] += 1
        if f.get("missing_or_weak_alt"):
            summary["missing_alt"] += 1
        if f.get("not_webp"):
            summary["not_webp"] += 1
        if f.get("weak_filename"):
