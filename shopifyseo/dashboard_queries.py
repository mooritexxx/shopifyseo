...
def fetch_page_detail(conn: sqlite3.Connection, handle: str) -> dict[str, Any] | None:
    ...
    # Find related collections that might link to this page (keyword match on title)
    all_collections = conn.execute("SELECT handle, title FROM collections ORDER BY title").fetchall()
    all_products = conn.execute("SELECT handle, title FROM products WHERE status = 'ACTIVE' ORDER BY title LIMIT 20").fetchall()
    all_pages = conn.execute("SELECT handle, title FROM pages WHERE handle != ? ORDER BY title LIMIT 10", (handle,)).fetchall()

    page_title_lower = page["title"].lower()
    related_collections = [
        dict(c) for c in all_collections
        if any(word in page_title_lower for word in c["title"].lower().split() if len(word) > 3)
    ][:10]

    return {
        "page": page,
        "related_collections": related_collections,
        "related_products": [dict(p) for p in all_products],
        "related_pages": [dict(p) for p in all_pages],
...
def fetch_blog_article_detail(conn: sqlite3.Connection, blog_handle: str, article_handle: str) -> dict[str, Any] | None:
    ...
    all_collections = conn.execute("SELECT handle, title FROM collections ORDER BY title").fetchall()
    all_products = conn.execute("SELECT handle, title FROM products WHERE status = 'ACTIVE' ORDER BY title LIMIT 20").fetchall()
    all_pages = conn.execute("SELECT handle, title FROM pages ORDER BY title LIMIT 10").fetchall()
    title_lower = (article.get("title") or "").lower()
    related_collections = [
        dict(c) for c in all_collections
        if any(word in title_lower for word in c["title"].lower().split() if len(word) > 3)
    ][:10]
